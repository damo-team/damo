...
2. 不同的属性分3种：
   ...
   2. 所有标签的`class`和`style`属性失效，React组件通过`className`和`style`来替代。而style属性只能是CSS的json形式，比如style={{backgroundColor: 'red'}}，其中`backgroundColor`而不是`background-color`,规则名已驼峰形式。
