...
1. Component, 如果把开面开发过程比喻为迪士尼花车，每个表演者是组件。
2. Store, 表演所需要的道具是就是状态容器，它就好比是道具仓库。
3. Selector是导演，导演决定给每个表演者分配哪些道具都是按照预设剧本来执行的。
4. 元数据，是整个剧组可以协作，帮助完成整个花车表演的的有利因素。
5. Model, 而道具来自于不同的地方，比如服装从服装仓库提供，交通工具、杂耍兵器都各自有仓库来管理，每个仓库就是一个model，是管理一类状态数据，同时具备更新、获取数据的方法。
