...
1. `FirstComponent`是一个组件实例），由\(\)括号包围的HTML标签及内容可以得到一个React组件实例。
2. `ReactDOM.render`把React组件实例的真实结构插入到页面body中。

> 注意ReactDOM.render\(React组件实例, 页面HTML结构DOM节点）
>
> 组件 vs 组件实例
>
> h1是一个组件，FirstComponent是一个组件实例
...
> 该函数实际上定义了一个React组件。
>
> 要返回组件实例，应该执行&lt;FirstComponent/&gt;，因为通过标签形式的调用才得到组件实例。
...
3. 在{}大括号包含的语句可以是JS表达式，也可以是React组件实例。
...
在HelloWorld的例子里`<h1>Hello World!</h1>` ，这是React的模板，它不是字符串（因为不需要用双引号包含），也不是HTML标签（使用上却基本和HTML无异），模板我们称之为JSX，每一段合法JSX是一个React组件实例，通过函数或者ES class来定义的JSX是一个React元素Element，同时也叫一个React组件。
