...
在HelloWorld的例子里`<h1>Hello World!</h1>` ，这是React的模板，它不是字符串（因为不需要用双引号包含），也不是HTML标签（使用上却基本和HTML无异），模板我们称之为JSX，每一段合法JSX是一个React组件实例，通过函数或者ES class来定义的JSX叫创建了一个React组件。
